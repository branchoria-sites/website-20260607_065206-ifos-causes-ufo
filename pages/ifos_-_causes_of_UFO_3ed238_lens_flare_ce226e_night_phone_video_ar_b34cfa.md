...
page_heading_title: Why night UFO videos fool the eye
source_count: 42
infographic_count: 3
planned_sections:
...
image: /assets/images/ifos_-_causes_of_UFO_3ed238_lens_flare_ce226e_night_phone_video_ar_b34cfa-Illustration-1-social.jpg
---
...
## Why bright points turn into glowing “orbs”

One of the most common misunderstandings in UFO videos is the assumption that a glowing blob represents the actual shape of an object.

In many cases, the camera is not resolving an object at all. It is recording a point source of light that is too small or too distant to produce a detailed image. Once that point becomes slightly out of focus, overexposed or affected by [lens flare]({{ 'lens-flare/' | relative_url }}), it expands into a bright disc or irregular glowing shape.

Several processes contribute:

* Slight defocus enlarges the light into a soft disc.
* Overexposure causes the bright area to bloom into neighbouring pixels.
* Internal [reflections]({{ 'reflections/' | relative_url }}) create ghost images and halos.
* Noise reduction smears the light across multiple frames.
* Digital sharpening can create artificial edges around the glow.

The resulting object may appear structured, pulsating or morphing even when the underlying source is a single steady light.
...
Humans estimate distance using contextual clues such as foreground objects, shadows, perspective and relative size. Night skies remove most of those clues.

When a bright light is recorded against a nearly featureless black background, viewers lose the ability to judge whether the source is tens of metres away or tens of kilometres away. A nearby drone, a distant aircraft and a bright planet may all appear as similarly sized points in a phone video.

This creates a cascade of interpretation errors.

If a witness assumes a light is nearby when it is actually distant, ordinary motion can appear extraordinary. A small movement of the camera can make the light seem to dart across the sky. Atmospheric turbulence can make it shimmer and wobble. A distant aircraft turning slightly can appear to accelerate suddenly.

The problem becomes worse when digital zoom is used. Unlike optical zoom, digital zoom enlarges pixels rather than revealing additional detail. The enlarged image often makes compression artefacts, noise and focus errors look like genuine structure. What appears to be a metallic craft or rotating object may simply be an enlarged and distorted point source.
...
Many reported UFO manoeuvres in night videos are difficult to separate from camera movement.

A bright light against a black sky provides no stable visual reference frame. If the phone shifts slightly, the viewer may interpret the movement as belonging to the object rather than to the camera.

Image stabilisation can make this even harder to judge. Modern phones digitally crop and reposition frames to reduce visible shaking. When a light source is the only prominent feature in the scene, stabilisation algorithms can introduce subtle motion artefacts that create the impression of drifting, hovering or sudden jumps.

Perspective also plays a role. A distant light observed from a moving vehicle can seem to pace the observer, stop abruptly or accelerate unexpectedly. Without landmarks in the frame, it is often impossible to reconstruct the true geometry of the event from the video alone.
...
* GPS location.
* Lens and zoom settings.
* Exposure information.
* Frame rate and resolution data.
* Device model and processing details.
